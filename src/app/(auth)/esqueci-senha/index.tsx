...
export default function EsqueciSenha() {
  ...
  async function handleRecuperarSenha() {
    ...
    if (emailLimpo === "") {
      setMensagemErro("Você precisa digitar seu e-mail antes de enviar.");
      return;
    }
    ...
    try {
      ...
      if (error.code === "auth/invalid-email") {
        setMensagemErro(
          "O formato do e-mail não é válido. Verifique se não esqueceu o @ ou .com",
        );
      } else if (error.code === "auth/user-not-found") {
        setMensagemErro(
          "Não encontramos nenhuma conta com este e-mail no sistema",
        );
      } else {
        setMensagemErro(
          "Houve um problema ao tentar recuperar a senha. Tente novamente depois",
        );
      }
    } finally {
      setCarregando(false);
